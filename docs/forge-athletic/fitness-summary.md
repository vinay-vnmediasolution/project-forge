# Project Athletic 80 / Forge Athletic

_Last updated: 8 August 2026_

## Athlete profile
- Age: 45
- Height: 176 cm
- Current weight range: 80.4–81.0 kg
- Latest recorded weight: 81.0 kg
- Latest recorded waist: 90 cm
- Primary goal: build muscle and strength while gradually reducing abdominal fat and waist size.
- Athletic goal: remain mobile and cricket-ready.

## Coaching and logging preferences
- Use a rolling training cycle rather than fixed weekdays.
- When a session is missed, continue with the next session in sequence.
- During workouts, keep feedback brief and provide the next set, target weight/reps and rest period.
- Use detailed reviews only after an exercise or full workout.
- Record RIR whenever possible.
- GitHub is the durable source of truth; chat memory is not sufficient.

## Weight conventions
- Barbell exercises: total weight including the bar.
- Dumbbell exercises: weight per dumbbell.
- Leg press: plates per side; sled resistance is not included.
- Smith-machine exercises: plates per side because effective bar resistance varies by machine.
- Cable and selectorised machines: weight displayed by the machine.

## Current strength benchmarks and PBs

### Push
- Flat barbell bench press top set: **80 kg × 6 @ 0 RIR**
- Flat barbell bench working sets: **70 kg × 6 × 2 @ ~2 RIR**, after 60 kg × 10
- Flat dumbbell bench press: **30 kg each × 10 × 2**
- Incline dumbbell press: **30 kg each × 8 × 3**
- Machine chest press: **60 kg × 6 × 2** after 40 kg × 10 calibration set
- Seated dumbbell shoulder press: **22.5 kg each × 8 × 3** historical benchmark; latest pain-managed session used 20 kg × 10 × 2
- Dumbbell lateral raise: **8 kg each × 10 × 3**
- Rope triceps pushdown: **25 kg × 10 × 2** after 22.5 kg × 10
- Overhead cable triceps extension: **21.25 kg × 10 × 2** after 17.5 kg × 12

### Pull
- Pull-ups: **bodyweight 6, 6, 6, 6**, latest final set @ 1 RIR
- Single-arm dumbbell row: **30 kg × 10 × 3 each side**
- Chest-supported row historical machine benchmark: **70 kg × 10, 10, 8**
- Chest-supported dumbbell row on incline bench: **30 kg each × 10 @ 1–2 RIR** top working set after 25 kg × 10 × 2
- Lat pulldown working load: **70 kg × 8 × 2 @ 1–2 RIR**; top set **80 kg × 8 @ 1 RIR**
- Seated row machine: **71 kg × 8 × 2 @ 1–2 RIR** after 61 kg × 10 @ 3–4 RIR
- Seated cable row historical benchmark: **50 kg × 10, 60 kg × 10, 65 kg × 8**
- Straight-arm pulldown: **21.25 kg × 12, 12, 10**
- Face pull: **15 kg × 15 × 3**
- Rear-delt fly machine working load: **45 kg × 10 @ 1–2 RIR**; top/test set **50 kg × 9 @ 0–1 RIR**
- EZ-bar curl working volume: **27.5 kg × 12 @ 1–2 RIR**; top set **32.5 kg × 8 @ 1 RIR**
- Hammer curl: **15 kg each × 8 × 3**

### Legs and core
- Free-bar back squat: **70 kg total × 6 × 4**
- Smith-machine squat: **30 kg per side × 8 × 2**; separate benchmark from free-bar squat
- Dumbbell Romanian deadlift: **27.5 kg each × 8 @ 1–2 RIR** after ramp sets of 22.5 × 10 and 25 × 10
- Leg press: **80 kg per side × 10 @ 1–2 RIR** after 60 × 10 and 70 × 10
- Leg extension: **23 kg × 12 × 3**
- Leg curl: **36 kg × 12, 41 kg × 10, 41 kg × 10**
- Seated calf raise: **51.3 kg effective resistance × 12, 10, 12** (20 kg each side plus 11.3 kg machine resistance)
- Standing Smith calf raise: **20 kg per side × 12 × 3**
- Hanging knee raise: **15 × 3**
- Plank: **60 seconds × 2** confirmed historical benchmark

## Progression method
- Primary target: finish most working sets with **1–2 RIR** and clean technique.
- 3+ RIR: consider a small increase when safe.
- 1–2 RIR: correct training load.
- 0 RIR: do not automatically increase; consolidate the load and improve reps/form.
- Increase only after the target sets and reps are repeatable with good technique.
- Separate working-weight benchmarks from top-set PBs.

## Nutrition and routine
- Daily protein target: approximately **150–170 g**, with **~160 g** as the practical midpoint.
- Creatine: **3–5 g once daily**; timing is not important and do not double-dose after training.
- Whey: use as needed to meet the daily protein target, commonly after training.
- Current daily supplements: Centrum Men and Vitamin D3 1000 IU.
- One vegetarian day each week, sometimes extending to two days; dairy and whey are acceptable.
- Common vegetarian proteins: Greek yogurt, whey, tofu, paneer, dal and edamame.
- Common pre-workout choice: banana 30–60 minutes before training.
- Typical recovery target: 7.5–8.5 hours sleep, 8,000–12,000 steps and 10–15 minutes mobility on recovery days.

## Daily nutrition log

### 8 August 2026
**Breakfast**
- Black coffee
- 3 boiled eggs
- Shake: 1 scoop protein powder, blueberries, strawberries, a few walnuts, chia seeds and 1½ cups Greek yoghurt
- Creatine: 5 g (daily dose taken)

**Pre-workout / supplements**
- 1 banana
- Magnesium: 1 tablet dissolved in one glass of water

### 6 August 2026
**Breakfast**
- 3 boiled eggs
- Shake with blueberries, strawberries, Greek yoghurt, chia seeds and protein powder
- Black coffee

## Body-metric tracking
- Record morning weight frequently under similar conditions.
- Use a 7-day average rather than reacting to one reading.
- Measure waist weekly at the navel, relaxed, before food and under consistent conditions.

## Health considerations
- Brief elbow discomfort occurred while setting up a shoulder press; it settled during the session.
- Stop or modify an exercise if pain is sharp, worsening or changes normal movement.

## Recent completed sessions

### Pull A
- Pull-ups: 6, 6, 6, 6
- Single-arm dumbbell row: 30 kg × 10 × 3 each side
- Lat pulldown: 60 × 10, 60 × 10, 70 × 8
- Seated cable row: 50 × 10, 60 × 10, 65 × 8
- EZ-bar curl: 27.5 × 10 × 3
- Hammer curl: 12.5 × 10 × 3

### Push B
- Barbell bench: 60 × 10 @ 3–4 RIR; 70 × 6 @ 2 RIR; 70 × 6 @ 2 RIR; **80 × 6 @ 0 RIR**
- Incline dumbbell press: 30 × 8 × 3
- Machine chest press: 40 × 10; 60 × 6; 60 × 6
- Lateral raise: 8 × 10 × 3
- Rope pushdown: 20 × 12; 22.5 × 10; 22.5 × 8
- Overhead cable extension: 15 × 10; 17.5 × 10; 17.5 × 10

### Optional Arms + Core
- EZ-bar curl: 30 × 8 × 3
- Hammer curl: 15 × 8 × 3
- Rope pushdown: 22.5 × 10; 25 × 10; 25 × 10
- Overhead cable extension: 17.5 × 12; 21.25 × 10; 21.25 × 10
- Rear-delt fly: 41 × 12; 45 × 10; 45 × 10
- Hanging knee raise: 15 × 3

### Legs B
- Smith squat: warm-up; 30 kg per side × 8 × 2; fourth set intentionally skipped
- Dumbbell RDL: 22.5 × 10; 25 × 10; 27.5 × 8
- Leg press: 60 kg per side × 10; 70 × 10; **80 × 10 @ 1–2 RIR**
- Leg curl: 36 × 12; 41 × 10; 41 × 10
- Standing Smith calf raise: **20 kg per side × 12 × 3**
- Hanging knee raise: **15 × 3**

### Pull B — 8 August 2026
- Pull-ups: **6 @ 1–2 RIR; 6 @ 1–2 RIR; 6 @ 1–2 RIR; 6 @ 1 RIR**
- Chest-supported dumbbell row on incline bench: **25 kg each × 10 @ 1–2 RIR; 25 × 10 @ 2–3 RIR; 30 × 10 @ 1–2 RIR**
- Lat pulldown: **70 kg × 8 @ 1–2 RIR; 70 × 8 @ 1–2 RIR; 80 × 8 @ 1 RIR**
- Seated row machine: **61 kg × 10 @ 3–4 RIR; 71 × 8 @ 1–2 RIR; 71 × 8 @ 1–2 RIR**
- Rear-delt fly machine: **45 kg × 10 @ 2–3 RIR; 50 × 9 @ 0–1 RIR; 45 × 10 @ 1–2 RIR**
- EZ-bar curl: **27.5 kg × 12 @ 1–2 RIR; 32.5 × 8 @ 1 RIR; 32.5 × 6** (RIR not provided for final set)
- Modification: programmed chest-supported row machine was unavailable/unclear, so incline-bench chest-supported dumbbell row was used and calibrated separately.
- Modification: seated row machine was used rather than the programmed cable-row variation; machine load is tracked separately.

## Current training state and next-session handoff
- Most recently completed: **Pull B — 8 August 2026**.
- Next planned workout in the rolling cycle: **Push A**, with recovery first if needed.
- Pull B progression notes:
  - Pull-ups: keep bodyweight × 6 × 4 and begin adding a seventh rep to one set when recovered.
  - Chest-supported dumbbell row: use **30 kg each × 8–10** as the next-session working benchmark for this incline-bench variation; confirm repeatability before increasing.
  - Lat pulldown: keep **70 kg** as the normal working load; **80 kg × 8 @ 1 RIR** is the current top-set benchmark and should be consolidated before progressing.
  - Seated row machine: use **71 kg × 8–10** as the current working prescription when using this machine.
  - Rear-delt fly: keep **45 kg × 10** as the working load; 50 kg remains a top/test load because 50 × 9 reached 0–1 RIR.
  - EZ-bar curl: use **27.5 kg for repeatable volume** and 32.5 kg as a top set; do not treat 32.5 kg as a 3-set working load yet.
